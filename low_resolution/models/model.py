...
class MCNN(nn.Module):
    def __init__(self, num_classes=10, hsic_training=False):
...
class MCNN_vib(nn.Module):
    def __init__(self, num_classes=5):
        ...
        self.fc = nn.Linear(self.k, self.num_classes)
...
class fc(nn.Module):
    def __init__(self, orig_fc):
        super(fc, self).__init__()
        self.fc2 = copy.deepcopy(orig_fc)
        self.fc1 = nn.Linear(self.fc2.in_features, self.fc2.in_features)
...
class VGG16(nn.Module):
    def __init__(self, n_classes, hsic_training=False, dataset='celeba'):
...
class VGG16_attr(nn.Module):
    def __init__(self, n_classes=2, dataset='celeba'):
        super(VGG16_attr, self).__init__()
...
class VGG16_vib(nn.Module):
    def __init__(self, n_classes=1000):
        ...
        self.st_layer = nn.Linear(self.feat_dim, self.k * 2)
        self.fc = nn.Linear(self.k, self.n_classes)
...
class VGG16_CXR_vib(nn.Module):
    def __init__(self, num_classes=7):
        super(VGG16_CXR_vib, self).__init__()
...
class VGG16_CXR(nn.Module):
    def __init__(self, num_classes=7, hsic_training=False):
        super(VGG16_CXR, self).__init__()

        self.hsic_training = hsic_training
        if self.hsic_training:
            blocks = make_layers(cfgs['D'], batch_norm=True)
            blocks[0][0] = nn.Conv2d(1, 64, kernel_size=(3, 3), stride=(1, 1), padding=(1, 1))
            self.layer1 = blocks[0]
            self.layer2 = blocks[1]
            self.layer3 = blocks[2]
            self.layer4 = blocks[3]
...
class ResNetCls(nn.Module):
    def __init__(self, nc=1, zdim=512, imagesize=64, nclass=8, resnetl=34, dropout=0):
